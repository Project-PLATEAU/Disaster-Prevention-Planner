      ...
      <div class="div">
        <div class="frame">
          <div class="frame-2">
            ...
            <form class="frame-4" method="post" action="{{route('org.human.exec')}}">
            @csrf
              <div class="sider">
                <div class="frame-5">
                  <div class="menu-logo">
                    <div class="div-2">
                      ...
                      <div class="menu">
                        ...
                        <div class="submenu" id="area_submenu">
                          ...
                          <div class="item-2">
                            <div class="element-nd-level-title"><a href="{{route('org.shelter.index')}}">地区一次避難場所</a></div>
                          </div>
              ...
              <div class="frame-6">
                <div class="frame-7">
                  <div class="heading-left">
                    <div class="wrapper">
                        <div class="title-4">
                            <a href="{{route('org.human.index')}}"><img class="arrow-left" src="{{asset('/img/arrowleft-2.svg')}}"/></a> 人的資源 登録</div></div>
                  </div>
                  <button class="button-2"><div class="text-2">保存</div></button>
                </div>
                <div class="frame-8">
                  <div class="vertical-form-item">
                    <div class="label"><div class="title-5">氏名</div></div>
                  ...
                  <div class="vertical-form-item">
                    <div class="label"><div class="title-6">資格・技能1</div></div>
                    <select class="select" name="skill1">
                      <option value="0">なし</option>
                      @foreach ($skill as $item)
                      <option value="{{$item->id}}" @isset($data) @if ($data->skill1 == $item->id)selected="selected" @endif @endisset>{{$item->name}}</option>
                      @endforeach
                    </select>
                  </div>
                  <div class="vertical-form-item">
                    <div class="label"><div class="title-6">資格・技能2</div></div>
                    <select class="select" name="skill2">
                      <option value="0">なし</option>
                      @foreach ($skill as $item)
                      <option value="{{$item->id}}" @isset($data) @if ($data->skill2 == $item->id)selected="selected" @endif @endisset>{{$item->name}}</option>
                      @endforeach
                    </select>
                  </div>
                  <div class="vertical-form-item">
                    <div class="label"><div class="title-6">資格・技能3</div></div>
                    <select class="select" name="skill3">
                      <option value="0">なし</option>
                      @foreach ($skill as $item)
                      <option value="{{$item->id}}" @isset($data) @if ($data->skill3 == $item->id)selected="selected" @endif @endisset>{{$item->name}}</option>
                      @endforeach
                    </select>
                  </div>
                  <div class="vertical-form-item">
                    <div class="label"><div class="title-6">資格・技能4</div></div>
                    <select class="select" name="skill4">
                      <option value="0">なし</option>
                      @foreach ($skill as $item)
                      <option value="{{$item->id}}" @isset($data) @if ($data->skill4 == $item->id)selected="selected" @endif @endisset>{{$item->name}}</option>
                      @endforeach
                    </select>
                  </div>
                  <div class="vertical-form-item-2">
                    <div class="label"><div class="text-wrapper-2">備考</div></div>
                    <textarea class="textarea" name="recital" placeholder="その他資格・技能等を記載">@isset($data){{$data->recital}}@endisset</textarea>
                  </div>
                </div>
              </div>
